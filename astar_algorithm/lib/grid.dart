...
enum GridGenerationFunction { random, backtracker, recursive }
...
class Grid extends ChangeNotifier {
  Grid(this.rows, this.columns, this.unitSize, this.starti, this.startj,
    ...
    width = unitSize * rows + 60;
    height = unitSize * columns + 40;
  }
  ...
  void fillWithWall() {
    ...
    staticNodes
        .forEach((l) => l.fillRange(0, nodeTypes[0].length - 1, gridBorder));
  ...
  void addNode(int i, int j, Brush type) {
    ...
    if (nodeTypes[i][j] == 0 || nodeTypes[i][j] == 4 || nodeTypes[i][j] == 5) {
      switch (type) {
        ...
        case Brush.open:
          nodeTypes[i][j] = 4;
          _updateStaticNode(i, j, ternaryPath);
          break;
        case Brush.closed:
          nodeTypes[i][j] = 5;
          nodes[i][j] = Positioned(
...
class GridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    Paint paint = Paint();
    Rect background = Rect.fromLTRB(-5, -5, width + 5, height);
